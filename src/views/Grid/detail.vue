          ...
          <Col span="8">
            <FormItem label="网格类型" prop="gridType">
              <Select v-model="form.gridType" placeholder="请选择网格类型">
                ...
                <Option value="7">临街商户</Option>
              </Select>
            </FormItem>
          </Col>
          <Col span="8">
            <FormItem label="管理员" prop="userId">
              <Input v-model="form.userId" placeholder="请选择网格管理员"></Input>
              <!-- <Select v-model="form.userId" placeholder="请选择网格管理员" prop="userId">
                <Option
                  v-for="(h, index) in userList"
                  :key="index"
                  :value="h.accountId+''"
                >{{ h.userId }}</Option>
              </Select> -->
...
export default {
  ...
  mounted() {
    ...
    if ("id" in this.$route.query) {
        ...
        if (rep.code === 200) {
          ...
          vue.$store.dispatch(actions.get.MECHANISM_PERSON, 1).then(rep1 => {
            if (rep1.code === 200) {
              ...
              this.$store
                .dispatch(actions.get.RESIDENT_USER_ROLEID, payload)
                .then(rep => {
                  if (rep.code === 200) {
      ...
      vue.form.value2 = [];
      // 获取所有的机构名称
      this.$store.dispatch(actions.get.MECHANISM_PERSON).then(rep => {
        if (rep.code === 200) {
      ...
      this.$refs[name].validate(valid => {
        if (valid) {
          ...
          if (this.listReview.length === 0) {
            ...
            if ("id" in this.$route.query) {
              this.$store
                .dispatch(actions.put.GRID_PERSON, payload)
                .then(rep => {
              ...
              this.$store
                .dispatch(actions.post.GRID_PERSON, payload)
                .then(rep => {
                  // 判断返回的消息，若返回得code为200 则认为操作成功
                  if (rep.data.code !== 200) {
                    this.$Message.error(rep.data.message);
