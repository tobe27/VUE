...
export default {
  ...
  mounted () {
    if ('id' in this.$route.query) {
      this.form=JSON.parse(sessionStorage.getItem("blacklist"))
    }
  ...
  methods: {
    getDetail (id) {
        var vue = this
      return this.$store.dispatch(actions.get.BLACK_PERSON, id).then( rep => {
          if(rep.code!=200){
  ...
  handleSubmit (name) {
      ...
      if ('id' in this.$route.query) {
        this.$store.dispatch(actions.put.BLACK_PERSON, payload).then(rep => {
            ...
            if("200" != rep.data.code){
                ...
                this.$router.push('/whitelist?tabname=2')
            }
        })
      } else {
        this.$store.dispatch(actions.post.BLACK_PERSON, payload).then(rep => {
            //判断返回的消息，若返回得code为200 则认为操作成功
            if("200" != rep.data.code){
                this.$Message.error(rep.data.message);
            }else{//否则返回一个警告窗 并把错误信息显示
