...
<div>
  <div class="residentpagetop">
    ...
    <div class="import">批量导入:&nbsp&nbsp&nbsp<Button class="import"  icon="ios-download-outline" @click="onImport">导入户籍信息</Button></div>
  </div>
  <div class="residentPage">
    ...
    <div class="residentButton-group">
      <Button class="create" @click="onAdd" type="primary" icon="ios-add">新建</Button>
      <!-- <Button class="import" @click="onImport">批量导入</Button> -->
    </div>
    <div class="table">
    ...
    <Modal v-model="showImportModal" title="批量导入" width="400px" :footer-hide="true" :styles="{top: '200px'}">
      ...
      <p>注：导入用户请先下载<a :href="uploadUrl+'/template/户籍导入模板.xlsx'">模板</a>，按要求填写、上传进行导入。</p>
...
export default {
  ...
  computed: {
    ...mapState({
      ...
      person: state => state.resident.person,
  ...
  methods: {
    ...
    onUpload (data) {
        if (data.code === 200) {
            ...
            this.onSearch()
            this.$Message.success('有' + data.data + '条记录导入成功')
        } else {
