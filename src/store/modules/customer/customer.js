...
export default {
  ...
  actions: {
    [ actions.get.CUSTOMER_LIST ] ({ commit }, payload) {
      ...
      let link = `/customer/info/list/user.accountId?pageNum=${payload.pageNum}&pageSize=${payload.pageSize}&accountId=`+user.accountId+'&roleId='+user.roles[0].roleId+'&orgCode='+user.orgCode
    ...
    [ actions.put.CUSTOMERS_PERSON ] (context, payload) {
      return axios.put('/customer/info/' + payload.id + '/status/' + payload.status)
    },
